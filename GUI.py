...
def display_troops(textes,sprites,Map):
	...
	for sprite in sprites:
		...
		textSurf, textRect = text_objects(str(pays.nb_troops), smallText)
...
class CurrentWindow():
	def __init__(self,window,turns):
		...
		self.pays_select=None # Selected country
	...
	def nb_units(self, value):
		if self.turns.phase==0: # Setter rules during deployment
			...
			if value<0:
				self._nb_units = 0
				raise ValueError('Too few troops',value)
			elif value>self.pays_select.nb_troops-1:
				self._nb_units = self.pays_select.nb_troops-1 # You can only attack/move with n-1 troops
				raise ValueError('Too much troops',value)
	...
	def display(self,fonction=None):
		...
		while display:
			...
			#	self.window.blit(sprite.map_pays,(0,0))
			self.window.blit(merged_pays,(0,0))
			for tmp in self.tmp:
			...
			try:
				if mouse_color != (0,0,0,0) and mouse_color != (0,0,0,255):
					...
					# 	sprite_bis=SpritePays(sprite.map_pays.copy(),"00.png") # Not clean
					# 	color_surface(sprite_bis,(1,1,1),150)
					# 	self.window.blit(sprite_bis.map_pays,(0,0))
					# 	pygame.display.flip()
					if id_pays_tmp != sprite_select:
						self.window.blit(sp_msq.map_pays,(0,0))
					...
					if self.turns.list_phase[self.turns.phase] == 'placement':
						#for event in pygame.event.get():
							...
							if click[0]==1:
								...
								if pays.id_player==self.turns.player_turn:
									...
									# try:
									# 	self.nb_units=self.players[self.turns.player_turn-1].nb_troops
									# except ValueError as e:
									# 	print(e.args)
								else:
									# Raise error
						...
						if click[0]==1 and not select: # Selection of attacking country
							...
							if pays1.id_player==self.turns.player_turn and pays1.nb_troops>1:
								self.nb_units=pays1.nb_troops-1
								self.tmp.append(sp_msq.map_pays)
								select=True 
								sprite_select=id_pays_tmp
						elif click[0]==1: # Selection of attacking country
							pays2=next((p for p in self.map.pays if p.id == id_pays_tmp), None)
							if atck_winmove and pays2 == pays_atck and pays1.nb_troops>1: # Move troops after successful attack
